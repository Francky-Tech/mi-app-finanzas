...
window.loadSpace = async function(spaceId) {
  ...
  window.buildNav(); window.switchTab('dashboard');
  return true;
};
...
window.addSpaceTx = async function(entry) {
  ...
  postChatActivity(`💳 ${uname()} registró ${enriched.tipo==='ingreso'?'+':'-'}${window.fmt(enriched.monto)} en ${enriched.categoria}`, 'tx', enriched);
};
...
function subscribeSpaceTx(spaceId) {
  ...
  SPACE.unsubTx = onSnapshot(
    ...
    snap => {
      ...
      if (t==='transactions') window.renderTransactions();
      if (t==='reports') window.renderReports();
    }
  );
}
...
function subscribeSpaceChat(spaceId) {
  SPACE.unsubChat?.();
  if (window._DEMO_MODE) { window.renderChat(); return; }
  SPACE.unsubChat = onSnapshot(
    query(collection(getDB(), 'spaces', spaceId, 'messages'), orderBy('createdAt','asc'), limit(100)),
    snap => { const msgs = snap.docs.map(d=>({id:d.id,...d.data()})); window.renderChat(msgs); updateChatBadge(msgs.length); }
  );
}
...
window.postChatMessage = async function(text) {
  ...
  if (window._DEMO_MODE) {
    ...
    msgs.push({...msg, id: Date.now()}); demoSaveList(spaceKey(SPACE.current.id,'messages'), msgs); window.renderChat(msgs); return;
  }
...
function window.renderChat(msgs) {
  ...
  msgs.forEach(m => {
    ...
    div.innerHTML=`<div style="width:28px;height:28px;border-radius:50%;background:${isMe?'var(--green)':'var(--s3)'};display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;color:${isMe?'#051209':'var(--text)'};flex-shrink:0">${m.authorAvatar||'?'}</div>
      ...
      <div style="background:${isMe?'var(--green-dim)':'var(--s2)'};border:1px solid ${isMe?'rgba(34,197,94,.3)':'var(--border)'};border-radius:${isMe?'12px 2px 12px 12px':'2px 12px 12px 12px'};padding:8px 12px;font-size:13px;line-height:1.5;color:var(--text)">${window.esc(m.text)}</div>
      ${m.attachedTx?`<div style="margin-top:4px;background:var(--s3);border:1px solid var(--border);border-radius:6px;padding:4px 8px;font-size:11px;color:var(--text2);font-family:'DM Mono',monospace">${m.attachedTx.tipo==='ingreso'?'+':'-'}${window.fmt(m.attachedTx.monto)} · ${m.attachedTx.categoria}</div>`:''}</div>`;
    c.appendChild(div);
  });
...
window.generateInviteLink = async function(role='member') {
  if (!SPACE.current || !canAdmin(SPACE.myRole)) { window.showToast('Solo admins pueden invitar','red'); return null; }
...
window.acceptInvite = async function(token) {
  ...
  if (SPACE.current?.id===invite.spaceId) { window.showToast('Ya eres miembro','yellow'); return false; }
  const cu = uid();
  if (window._DEMO_MODE) {
    const raw=localStorage.getItem('ap_space_'+invite.spaceId); if(!raw){window.showToast('Espacio no encontrado','red');return false;}
...
window.getSpaceHeader = function() {
  ...
  return `<div style="display:flex;align-items:center;gap:10px;background:var(--green-dim);border:1px solid rgba(34,197,94,.2);border-radius:12px;padding:10px 16px;margin-bottom:16px">
    ...
    <div style="flex:1"><div style="font-family:'Syne',sans-serif;font-size:14px;font-weight:700">${window.esc(SPACE.current.name)}</div>
...
window.openSpacePanel  = function() { document.getElementById('spacePanelModal')?.classList.add('open'); window.renderSpacePanel(); };
...
window.renderSpacePanel = async function() {
  ...
  ${members.map(([mid,role])=>{const info=SPACE.memberCache[mid]||{displayName:mid===uid()?uname():'Miembro',avatarLetter:'?'};return`<div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--border)">
    ...
    <div style="flex:1"><div style="font-size:13px;font-weight:500">${window.esc(info.displayName)}${mid===uid()?' <span style="font-size:11px;color:var(--text3)">(tú)</span>':''}</div></div>
...
window.copyInviteLink = async function() {
  ...
  try{await navigator.clipboard.writeText(link);window.showToast('Link copiado ✓','green');}catch{window.showToast('Link generado','blue');}
};
...
window.changeRole = async function(memberId,newRole) {
  ...
  window.showToast('Rol actualizado ✓','green'); window.renderSpacePanel();
};
...
window.removeMember = async function(memberId) {
  ...
  window.showToast('Miembro eliminado','red'); window.renderSpacePanel();
};
