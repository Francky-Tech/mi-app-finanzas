...
// IMPORTANTE: Este archivo es type="module" con scope aislado.
// Todas las funciones de app.js se acceden via window.X()
// Los imports DEBEN ir primero en un módulo ES.
// ============================================================
...
function getUser() {
  ...
  const apps = getApps();
  if (apps.length) {
    const u = getAuth(apps[0]).currentUser;
    if (u && u.uid) return u;
  }
  return null;
}
...
window.createSpace = async function(name, emoji) {
  ...
  if (!currentUid) {
    window.showToast('Error de sesión. Recarga la página.', 'red');
    return null;
  }
  ...
  const space = {
    ...
    members,
    config: {
      tabs: (window.APP_CONFIG && window.APP_CONFIG.tabs) || [],
      limiteOcio: (window.APP_CONFIG && window.APP_CONFIG.limiteOcio) || 500000,
    },
  ...
  if (window._DEMO_MODE) {
    const list = demoGet('ap_my_spaces_' + currentUid);
...
window.loadSpace = async function(spaceId) {
  ...
  if (!spaceData) {
    window.showToast('Espacio no encontrado', 'red');
    return false;
  }
...
window.leaveSpace = async function() {
  ...
  const cu = uid();
  const results = await Promise.all([window._db.getGastos(cu), window._db.getMetas(cu)]);
  window.gastos = results[0] || [];
  window.metas  = results[1] || [];

  window.showToast('Saliste del espacio compartido', 'yellow');
  window.buildNav();
  window.switchTab('dashboard');
};
...
window.addSpaceTx = async function(entry) {
  ...
  const enriched = Object.assign({}, entry, {
    id: Date.now(),
    spaceId: window.SPACE.current.id,
    createdBy: uid(),
    authorName: uname(),
  ...
  const fmtFn = window.fmt || function(n) { return '$' + Number(n||0).toLocaleString('es-CO'); };
...
async function loadSpaceTx(spaceId) {
  if (window._DEMO_MODE) {
    window.gastos = demoGet(spaceKey(spaceId, 'gastos'));
    return;
  }
...
function subscribeSpaceChat(spaceId) {
  ...
  window.SPACE.unsubChat = onSnapshot(
    ...
    snap => {
      ...
      if (badge) badge.style.display = msgs.length > 0 ? 'inline' : 'none';
    }
  );
}
...
window.postChatMessage = async function(text) {
  ...
  await addDoc(
    collection(getDB(), 'spaces', window.SPACE.current.id, 'messages'),
    Object.assign({}, msg, { createdAt: serverTimestamp() })
  );
};
...
async function postChatActivity(text, type, attachedTx) {
  ...
  const msg = {
    text, uid: uid(), authorName: uname(), authorAvatar: uavatar(),
    type, attachedTx: attachedTx || null, createdAt: new Date().toISOString(),
  };
  if (window._DEMO_MODE) {
    const msgs = demoGet(spaceKey(window.SPACE.current.id, 'messages'));
    msgs.push(Object.assign({ id: Date.now() }, msg));
...
// Función local de renderizado — expuesta en window al final
function renderChatLocal(msgs) {
  const c = document.getElementById('chatMessages');
  if (!c) return;

  const escFn = window.esc || function(s) {
    return String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
  };
  const fmtFn = window.fmt || function(n) {
    return '$' + Number(n||0).toLocaleString('es-CO');
  };

  if (!msgs) {
    msgs = (window._DEMO_MODE && window.SPACE.current)
      ? demoGet(spaceKey(window.SPACE.current.id, 'messages'))
      : [];
  }

  c.innerHTML = msgs.length
    ? ''
    : '<div style="text-align:center;padding:20px;color:var(--text3);font-size:13px">Sé el primero en escribir algo 👋</div>';
  ...
  msgs.forEach(function(m) {
    ...
    div.innerHTML =
      ...
      (isMe ? '#051209' : 'var(--text)') + ';flex-shrink:0">' + (m.authorAvatar || '?') + '</div>' +
      '<div style="max-width:75%">' +
        '<div style="font-size:10px;color:var(--text3);margin-bottom:3px;' + (isMe ? 'text-align:right' : '') + '">' +
          (isMe ? 'Tú' : m.authorName) + '</div>' +
        '<div style="background:' + (isMe ? 'var(--green-dim)' : 'var(--s2)') +
          ';border:1px solid ' + (isMe ? 'rgba(34,197,94,.3)' : 'var(--border)') +
          ';border-radius:' + (isMe ? '12px 2px 12px 12px' : '2px 12px 12px 12px') +
          ';padding:8px 12px;font-size:13px;line-height:1.5;color:var(--text)">' + escFn(m.text) + '</div>' +
        txHtml +
      '</div>';
...
// Exponer para que spaces-ui.js pueda llamarla
window.renderChat = renderChatLocal;

// ── Invitaciones ─────────────────────────────────────────────
window.generateInviteLink = async function(role) {
  role = role || 'member';
  if (!window.SPACE.current || !canAdmin(window.SPACE.myRole)) {
    window.showToast('Solo admins pueden invitar', 'red');
    return null;
  }
...
window.acceptInvite = async function(token) {
  let invite;
  if (window._DEMO_MODE) {
    const r = localStorage.getItem('ap_invite_' + token);
    invite = r ? JSON.parse(r) : null;
  } else {
    const db = await getDBReady();
    const s  = await getDoc(doc(db, 'invites', token));
  ...
  if (window.SPACE.current && window.SPACE.current.id === invite.spaceId) {
    window.showToast('Ya eres miembro', 'yellow'); return false;
  }
...
function buildMemberCache() {
  if (!window.SPACE.current) return;
  const members = window.SPACE.current.members || {};
...
window.saveSpaceData = async function() {
  ...
  if (window._DEMO_MODE) {
    demoSet(spaceKey(window.SPACE.current.id, 'metas'), window.metas);
    return;
  }
...
window.saveSpacePerfil = async function() {
  ...
  if (window._DEMO_MODE) {
    ...
    if (raw) {
      ...
      sp.config = window.APP_CONFIG;
      localStorage.setItem('ap_space_' + window.SPACE.current.id, JSON.stringify(sp));
    }
    return;
  }
  await updateDoc(doc(getDB(), 'spaces', window.SPACE.current.id), {
...
window.getSpaceHeader = function() {
  if (!window.SPACE.current) return '';
  const escFn   = window.esc || function(s) { return String(s); };
  ...
  return '<div style="display:flex;align-items:center;gap:10px;background:var(--green-dim);border:1px solid rgba(34,197,94,.2);border-radius:12px;padding:10px 16px;margin-bottom:16px">' +
    ...
    '<div style="flex:1">' +
      '<div style="font-family:\'Syne\',sans-serif;font-size:14px;font-weight:700">' + escFn(window.SPACE.current.name) + '</div>' +
      '<div style="font-size:11px;color:var(--text2)">' + members.length + ' miembro' + (members.length !== 1 ? 's' : '') +
        ' · rol: <strong style="color:var(--green)">' + window.SPACE.myRole + '</strong></div>' +
    '</div>' +
    '<button class="btn-icon" onclick="window.openSpacePanel()" style="font-size:12px;padding:5px 10px">⚙️ Gestionar</button>' +
    '<button class="btn-icon" onclick="window.leaveSpace()" style="font-size:12px;padding:5px 10px;color:var(--red)">✕ Salir</button>' +
  '</div>';
};

window.openSpacePanel  = function() {
  const m = document.getElementById('spacePanelModal');
...
window.renderSpacePanel = async function() {
  ...
  const rows = members.map(function(entry) {
    const mid  = entry[0];
    const role = entry[1];
    const info = window.SPACE.memberCache[mid] || {
      displayName: mid === uid() ? uname() : 'Miembro',
      avatarLetter: '?',
    };
    let controls = '';
    if (isAdmin && mid !== uid() && role !== 'owner') {
      controls = '<select onchange="window.changeRole(\'' + mid + '\',this.value)" style="width:90px;font-size:11px;padding:3px 6px">' +
        ROLES_ORDER.filter(r => r !== 'owner').map(r =>
          '<option value="' + r + '"' + (r === role ? ' selected' : '') + '>' + r + '</option>'
        ).join('') + '</select>';
      if (isOwner) {
        controls += '<button class="btn-icon" onclick="window.removeMember(\'' + mid + '\')" style="color:var(--red);font-size:11px">✕</button>';
      }
    }
    return '<div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--border)">' +
      '<div style="width:32px;height:32px;border-radius:50%;background:' + (mid === uid() ? 'var(--green)' : 'var(--s3)') +
        ';display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:700;color:' +
        (mid === uid() ? '#051209' : 'var(--text)') + ';flex-shrink:0">' + info.avatarLetter + '</div>' +
      '<div style="flex:1"><div style="font-size:13px;font-weight:500">' + escFn(info.displayName) +
        (mid === uid() ? '<span style="font-size:11px;color:var(--text3)"> (tú)</span>' : '') + '</div></div>' +
      '<span style="font-size:11px;padding:2px 8px;border-radius:99px;background:' +
        (role === 'owner' ? 'var(--yellow-dim)' : role === 'admin' ? 'var(--blue-dim)' : 'var(--green-dim)') +
        ';color:' + (role === 'owner' ? 'var(--yellow)' : role === 'admin' ? 'var(--blue)' : 'var(--green)') + '">' + role + '</span>' +
      controls + '</div>';
  }).join('');
  ...
  if (isAdmin) {
    inviteHtml =
      '<div style="margin-bottom:20px">' +
        '<div style="font-size:11px;font-weight:600;color:var(--text3);text-transform:uppercase;letter-spacing:.5px;margin-bottom:10px">Invitar miembro</div>' +
        '<div style="display:flex;gap:8px;margin-bottom:8px">' +
          ...
          '</select>' +
          '<button class="btn btn-primary btn-sm" onclick="window.copyInviteLink()">🔗 Generar link</button>' +
        '</div>' +
      ...
      '</div>' +
      '<div>' +
        '<div style="font-size:11px;font-weight:600;color:var(--text3);text-transform:uppercase;letter-spacing:.5px;margin-bottom:10px">Sincronizar perfil</div>' +
        '<button class="btn btn-ghost btn-sm" onclick="window.saveSpacePerfil()">📤 Sincronizar</button>' +
      '</div>';
  }

  panel.innerHTML =
    '<div style="margin-bottom:20px">' +
      '<div style="font-size:11px;font-weight:600;color:var(--text3);text-transform:uppercase;letter-spacing:.5px;margin-bottom:10px">Miembros (' + members.length + ')</div>' +
      rows +
    '</div>' +
    inviteHtml;
};
...
window.copyInviteLink = async function() {
  ...
  const link   = await window.generateInviteLink(roleEl ? roleEl.value : 'member');
  if (!link) return;
  const box = document.getElementById('inviteLinkBox');
  if (box) { box.style.display = 'block'; box.textContent = link; }
  try {
    await navigator.clipboard.writeText(link);
    window.showToast('Link copiado ✓', 'green');
  } catch(e) {
    window.showToast('Link generado — cópialo manualmente', 'blue');
  }
};
...
window.changeRole = async function(memberId, newRole) {
  ...
  if (window._DEMO_MODE) {
    ...
    if (raw) {
      const sp = JSON.parse(raw);
      sp.members[memberId] = newRole;
      localStorage.setItem('ap_space_' + window.SPACE.current.id, JSON.stringify(sp));
      window.SPACE.current.members[memberId] = newRole;
    }
  } else {
    const upd = {};
    upd['members.' + memberId] = newRole;
...
window.removeMember = async function(memberId) {
  ...
  if (window._DEMO_MODE) {
    ...
    if (raw) {
      const sp = JSON.parse(raw);
      delete sp.members[memberId];
      localStorage.setItem('ap_space_' + window.SPACE.current.id, JSON.stringify(sp));
      delete window.SPACE.current.members[memberId];
    }
  } else {
    const upd = {};
    upd['members.' + memberId] = null;
    await updateDoc(doc(getDB(), 'spaces', window.SPACE.current.id), upd);
    delete window.SPACE.current.members[memberId];
  }
